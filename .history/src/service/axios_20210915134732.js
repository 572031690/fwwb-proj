...
function getMethod (ajaxData, data, sendType) {
  ...
  let type = ''
  if (sendType === 'params') type = 'params'
  for (const i in ajaxData[sendType]) {
    ajaxData[sendType][i] = data[type][i]
  }
  if (sendType === 'params') { return get(ajaxData.path, ajaxData[sendType]) }
  if (sendType === 'data') { return post(ajaxData.path, ajaxData[sendType]) }
}
