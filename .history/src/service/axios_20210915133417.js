...
function getMethod (ajaxData, data, type) {
  let sendType = ''
  if (type === 'get') sendType = 'params'
  if (type === 'post') sendType = 'data'
  ...
  console.log(ajaxData[sendType], 'ajaxData')
  console.log([type](ajaxData.path, ajaxData[sendType]), 'sendDatasendDatasendData')

  return [type](ajaxData.path, ajaxData[sendType])
}
...
var sendAxios = function (url, data, sendHeader) {
  ...
  if (ajaxData.method === 'GET') {
    ...
    const sendData = getMethod(ajaxData, data, 'get')
    return sendData
