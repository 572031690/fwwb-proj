<template>
  <div id="Login"  ref="loginview">
        ...
        <div class="top-backhome"  >
          <img src="../assets/login-top.png" height="50px" style="vertical-align: middle;" >
          <span >登陆界面</span>
        </div>
		    <div class="login-top" >
		      <img src="../assets/center-login.png" width="100%">
        ...
        <transition name="centerboxout">
          ...
          <div class="centerbox" v-show="centerbox">
            <div class="center-heart">
              ...
              <form>
                <div >
                  <input type="text"  class="name" placeholder="username" ref="logintext" v-model="logindata.uname"/>
                  <div class="logintip">
                  {{tips1}}
                  </div>
                </div>
                <div >
                  <input type="password"  class="password" ref="passwordeye" placeholder="password" v-model="logindata.pass"/>
                  <img :src="studyDataPic" class="passeyes" ref="passeyes" @click="eyeschange()">
                  <div class="logintip">
                  {{tips2}}
                  </div>
                </div>
                <div class="regiserbox">
                  <span>验证码：</span>
                  <input type="text"  class="regisertext" v-model="inputVal"  />
                  <!-- 验证码组件 -->
                ...
                <div class="loginbutton">
                  ...
                  <button type="primary" id="login" @click="login()" :plain="true" >登陆</button>
                  <button type="button" id="regiser" @click="seeData">注册</button>
                </div>
              </form>
            </div>
          </div>
                            ...
                            <el-dialog  :visible.sync="dialogFormVisible" :modal-append-to-body='false' :close-on-click-modal="false" :show-close="false" center  width="35%">
								...
								<el-form :model="form" :rules="rules" ref="form" label-width="120px" class="demo-ruleForm">
                                    ...
                                    <el-form-item label="密码" prop="pass">
                                        <el-input v-model="form.pass" style="width:400px" type="password"></el-input>
                                    </el-form-item>
									<el-form-item label="确认密码" prop="repass">
                                        <el-input v-model="form.repass" style="width:400px" type="password"></el-input>
                                    </el-form-item>
									...
									<el-form-item label="验证码" prop="rescode" >
                                        <el-input type="age" v-model.number="form.rescode" auto-complete="off" style="width:200px"></el-input>

										<el-button type="primary" plain size="small" @click="sendresCode()" :class="{'disabled-style':getCodeBtnDisable}" :disabled="getCodeBtnDisable" style="width:100px; margin-left:12px;height:30px;text-align: center;">{{codenum}}</el-button>
                                    </el-form-item>
...
export default {
  ...
  data () {
    ...
    return {
      ...
      studyDataPic: require('@/assets/closeeye.png'),
      ...
      this.$refs[formName].validate((valid) => {
        if (valid) {
          ...
          this.$confirm('是否确定保存并注册账户?', '提示', {
            ...
            if (this.form.tel == this.phone.num && this.form.rescode == this.phone.code) {
                		this.register()
            } else {
    ...
    // 密码眼睛切换
      	eyeschange () {
        	if (this.eyesflag) {
          		this.studyDataPic = require('@/assets/eye.png')
          		this.eyesflag = !this.eyesflag
          		this.$refs.passwordeye.type = 'text'
        	} else {
          		this.studyDataPic = require('@/assets/closeeye.png')
          		this.eyesflag = !this.eyesflag
    ...
    login () {
      ...
      if (!this.$refs.logintext.value) {
        ...
        const url = '/web/login'
        this.$ajax.get(url, { params: { username: this.logindata.uname, password: this.logindata.pass } })
          .then(res => {
            // if(res.data==0){
            ...
            if (res.data.code == 100) {
              // console.log(res);
              window.sessionStorage.setItem('storeData', res.data.user.username)// 将数据存储到浏览器内嵌的数据库内
              window.sessionStorage.setItem('sData', res.data.user.departmentid)// 将数据存储到浏览器内嵌的数据库内
    ...
    sendresCode () {
      ...
      // console.log(params);
      // 调用获取短信验证码接口
      this.$ajax.get('/getCode', { params: { phone: this.form.tel } }).then(res => {
        res = res.data
        console.log(res)
        if (res.code) {
...
<style >
  ...
  .centerboxout-enter{
	...
	.name{
		background-image:url(../assets/登陆.png) ; /* 设置小图标 */
  ...
  input::-webkit-input-placeholder {
	...
	.password{
		background-image:url(../assets/密码.png) ; /* 设置小图标 */
		...
		border-radius: 5px;
		outline: none;
		/* background-image: url(../assets/闭眼%20.png); */
		/* margin: 10px 0; */
	}
	.regiserbox{
