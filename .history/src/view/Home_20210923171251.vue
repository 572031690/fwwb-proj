...
export default {
  ...
  methods: {
    ...
    changearrow (index) {
      ...
      if (this.arrowflag) {
        this.$refs.tipsarrowappro[0].style.transform = 'rotate(270deg)'
        this.arrowflag = !this.arrowflag
      } else {
        this.$refs.tipsarrowappro[0].style.transform = 'rotate(90deg)'
        this.arrowflag = !this.arrowflag
      }
    },
    // 右边栏三条杠点击事件
    changehomeimg () {
      ...
      setTimeout(function () {
        ...
        const arrowData = ['tipsarrowappro']
    ...
    initType (bool) {
      for (let i = 1; i <= 5; i++) {
        ...
        if (this.routerList[i].type === 'tips') this.arrowData.push(this.routerList[i].arrowRef)
